...
| Parity gate over FX-converted measures | Every `SUMX(… / IFERROR(LOOKUPVALUE(rate, cur[Currency], SELECTEDVALUE(cur[Currency], BLANK()), date), 0))` measure is **NaN** with no single currency in filter context (LOOKUPVALUE no-match returns BLANK; IFERROR never fires) — a NaN-vs-NaN comparison PASSES vacuously (20 of 111 measures, 1,027 of 6,132 rows here). Compare non-finite values as strings, re-run those measures with one currency pinned via `TREATAS`, require a non-finite count of 0, and report verified-vs-undefined coverage. |
| Cloning a model onto its OWN `cache.abf` | Copy the model folder MINUS `.pbi\localSettings.json` (it pins the published datasetId), fresh `logicalId`s, partition M byte-identical — every existing table's data survives the first open (30 of 31 partitions `State 1`, only the added table `State 3`); one refresh finishes it. Poll readiness by table NAMES (must-have / must-not-have), never by count: Desktop restores the OLD schema first and both caches can hold the same table count. |
| About to divide by a quantity / lot-size field, or a reconstructed cost is absurd | **Profile the candidate field's VALUES on a real export first** (nulls, count of the default, count of > 1, min/median/max) and **compare against the source system's OWN computed figure** before blaming its master data. Two same-shaped fields sat side by side here: the charges quantity (0 or 1 on 99.5 % of rows) and the price/calculation quantity (> 1 on 30 %, up to 104,400) — round 1 verified only that the first EXISTED and moved the total 0.24 %; round 2 used the second and moved it −98.65 %. The ERP's own unit cost (0.36 vs a reconstructed 250) proved the defect was in the reconstruction, not the data. Write the rule as `fieldA > default → fieldB > default → exclude + warn`, never `else <default>` — an "accept 1 when fixed charges > 0" clause let 61.8 % of the defect through. reference.md §2026-09-06. |
